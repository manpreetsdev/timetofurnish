                            ...
                            <div class="row">
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control aiz-selectpicker" data-container="body" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][header_tag]">
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group mb-0">
                                        ...
                                        <select class="form-control aiz-selectpicker" data-container="body" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][text_align]">
            ...
            <div class="ttf-section-settings ttf-section-settings--widget">
                <details class="ttf-setting-group">
                    ...
                    <div class="ttf-setting-group__body">
                        @if ($isRichText)
                            <div class="row">
                                <div class="col-6"><div class="form-group"><label>{{ translate('Text Align') }}</label><select class="form-control aiz-selectpicker" data-container="body" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][text_align]"><option value="left" @selected(($widget['text_align'] ?? '') === 'left')>{{ translate('Left') }}</option><option value="center" @selected(($widget['text_align'] ?? '') === 'center')>{{ translate('Center') }}</option><option value="right" @selected(($widget['text_align'] ?? '') === 'right')>{{ translate('Right') }}</option></select></div></div>
                        ...
                        @if ($isSplit)
                            <div class="row">
                                <div class="col-12"><div class="form-group"><label>{{ translate('Desktop Layout') }}</label><select class="form-control aiz-selectpicker" data-container="body" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][layout]"><option value="image_left" @selected(($widget['layout'] ?? '') === 'image_left')>{{ translate('Image Left / Content Right') }}</option><option value="image_right" @selected(($widget['layout'] ?? '') === 'image_right')>{{ translate('Content Left / Image Right') }}</option></select></div></div>
                                <div class="col-6"><div class="form-group"><label>{{ translate('iPad Order') }}</label><select class="form-control aiz-selectpicker" data-container="body" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][tablet_stack_order]"><option value="content_first" @selected(($widget['tablet_stack_order'] ?? '') === 'content_first')>{{ translate('Heading First') }}</option><option value="image_first" @selected(($widget['tablet_stack_order'] ?? '') === 'image_first')>{{ translate('Image First') }}</option></select></div></div>
                                <div class="col-6"><div class="form-group"><label>{{ translate('Phone Order') }}</label><select class="form-control aiz-selectpicker" data-container="body" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][mobile_stack_order]"><option value="content_first" @selected(($widget['mobile_stack_order'] ?? '') === 'content_first')>{{ translate('Heading First') }}</option><option value="image_first" @selected(($widget['mobile_stack_order'] ?? '') === 'image_first')>{{ translate('Image First') }}</option></select></div></div>
                            </div>
                        @endif

                        @if ($isFullWidth)
                            <div class="row">
                                <div class="col-12"><div class="form-group"><label>{{ translate('Display Mode') }}</label><select class="form-control aiz-selectpicker" data-container="body" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][display_mode]"><option value="content_only" @selected(($widget['display_mode'] ?? '') === 'content_only')>{{ translate('Content Only') }}</option><option value="image_only" @selected(($widget['display_mode'] ?? '') === 'image_only')>{{ translate('Image Only') }}</option><option value="content_image" @selected(($widget['display_mode'] ?? '') === 'content_image')>{{ translate('Content + Image') }}</option></select></div></div>
                                <div class="col-12"><div class="form-group"><label>{{ translate('Desktop Position') }}</label><select class="form-control aiz-selectpicker" data-container="body" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][image_position]"><option value="top" @selected(($widget['image_position'] ?? '') === 'top')>{{ translate('Image Top') }}</option><option value="bottom" @selected(($widget['image_position'] ?? '') === 'bottom')>{{ translate('Image Bottom') }}</option></select></div></div>
                                <div class="col-6"><div class="form-group mb-0"><label>{{ translate('iPad Position') }}</label><select class="form-control aiz-selectpicker" data-container="body" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][tablet_image_position]"><option value="top" @selected(($widget['tablet_image_position'] ?? '') === 'top')>{{ translate('Image Top') }}</option><option value="bottom" @selected(($widget['tablet_image_position'] ?? '') === 'bottom')>{{ translate('Image Bottom') }}</option></select></div></div>
                                <div class="col-6"><div class="form-group mb-0"><label>{{ translate('Phone Position') }}</label><select class="form-control aiz-selectpicker" data-container="body" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][mobile_image_position]"><option value="top" @selected(($widget['mobile_image_position'] ?? '') === 'top')>{{ translate('Image Top') }}</option><option value="bottom" @selected(($widget['mobile_image_position'] ?? '') === 'bottom')>{{ translate('Image Bottom') }}</option></select></div></div>
                            </div>
                        @endif

                        @if ($isGrid)
                            <div class="row">
                                <div class="col-6"><div class="form-group"><label>{{ translate('Columns') }}</label><select class="form-control aiz-selectpicker" data-container="body" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][columns]">@foreach ([2,3,4] as $columnCount)<option value="{{ $columnCount }}" @selected((string) ($widget['columns'] ?? '3') === (string) $columnCount)>{{ $columnCount }}</option>@endforeach</select></div></div>
                        ...
                        @if ($isImage)
                            <div class="row">
                                <div class="col-12"><div class="form-group"><label>{{ translate('Alignment') }}</label><select class="form-control aiz-selectpicker" data-container="body" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][image_align]"><option value="left" @selected(($widget['image_align'] ?? 'center') === 'left')>{{ translate('Left') }}</option><option value="center" @selected(($widget['image_align'] ?? 'center') === 'center')>{{ translate('Center') }}</option><option value="right" @selected(($widget['image_align'] ?? 'center') === 'right')>{{ translate('Right') }}</option></select></div></div>
                                <div class="col-6"><div class="form-group mb-0"><label>{{ translate('Width (%)') }}</label><input type="number" class="form-control" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][image_width]" value="{{ $widget['image_width'] ?? '100' }}" min="10" max="100"></div></div>
                                <div class="col-6"><div class="form-group mb-0"><label>{{ translate('Height (px)') }}</label><input type="number" class="form-control" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][image_height]" value="{{ $widget['image_height'] ?? '' }}" placeholder="{{ translate('Auto') }}"></div></div>
                            </div>
                        @endif

                        @if ($isButton)
                            <div class="row">
                                <div class="col-6"><div class="form-group"><label>{{ translate('Alignment') }}</label><select class="form-control aiz-selectpicker" data-container="body" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][button_align]"><option value="left" @selected(($widget['button_align'] ?? 'left') === 'left')>{{ translate('Left') }}</option><option value="center" @selected(($widget['button_align'] ?? '') === 'center')>{{ translate('Center') }}</option><option value="right" @selected(($widget['button_align'] ?? '') === 'right')>{{ translate('Right') }}</option></select></div></div>
                                ...
                                <div class="col-6"><div class="form-group"><label>{{ translate('Font Size') }}</label><input type="text" class="form-control" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][button_font_size]" value="{{ $widget['button_font_size'] ?? '16px' }}"></div></div>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control aiz-selectpicker" data-container="body" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][title_font_family]">
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control aiz-selectpicker" data-container="body" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][body_font_family]">
                            ...
                            <div class="col-6"><div class="form-group mb-2"><label>{{ translate('H Size') }}</label><input type="text" class="form-control" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][title_font_size]" value="{{ $widget['title_font_size'] ?? '' }}" placeholder="28px"></div></div>
                            <div class="col-6"><div class="form-group mb-2"><label>{{ translate('H Line H') }}</label><input type="text" class="form-control" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][title_line_height]" value="{{ $widget['title_line_height'] ?? '' }}" placeholder="33px"></div></div>
                            <div class="col-6"><div class="form-group mb-0"><label>{{ translate('B Size') }}</label><input type="text" class="form-control" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][body_font_size]" value="{{ $widget['body_font_size'] ?? '' }}" placeholder="18px"></div></div>
                            <div class="col-6"><div class="form-group mb-0"><label>{{ translate('B Line H') }}</label><input type="text" class="form-control" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][body_line_height]" value="{{ $widget['body_line_height'] ?? '' }}" placeholder="31px"></div></div>
                        </div>

                        <div class="row mt-2">
                            <div class="col-6"><div class="form-group mb-2"><label>{{ translate('Heading') }}</label><input type="color" class="form-control" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][title_color]" value="{{ $widget['title_color'] ?? '#2c2218' }}"></div></div>
                            <div class="col-6"><div class="form-group mb-2"><label>{{ translate('Accent') }}</label><input type="color" class="form-control" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][accent_color]" value="{{ $widget['accent_color'] ?? '#C27325' }}"></div></div>
                            <div class="col-6"><div class="form-group mb-2"><label>{{ translate('Sub Heading') }}</label><input type="color" class="form-control" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][subtitle_color]" value="{{ $widget['subtitle_color'] ?? '#5b4839' }}"></div></div>
                            <div class="col-6"><div class="form-group mb-2"><label>{{ translate('Body Text') }}</label><input type="color" class="form-control" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][body_color]" value="{{ $widget['body_color'] ?? '#393939' }}"></div></div>
                            <div class="col-6"><div class="form-group mb-2"><label>{{ translate('Highlight') }}</label><input type="color" class="form-control" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][highlight_color]" value="{{ $widget['highlight_color'] ?? '#C27325' }}"></div></div>
                            @if ($isSplit)
                                <div class="col-6"><div class="form-group mb-2"><label>{{ translate('Check Icon') }}</label><input type="color" class="form-control" name="builder[sections][{{ $groupIndex }}][widgets][{{ $widgetIndex }}][check_icon_color]" value="{{ $widget['check_icon_color'] ?? '#C27325' }}"></div></div>
                            @endif
                        </div>
                    </div>
                </details>
